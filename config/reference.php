 ...
 *     }>,
 * }
 * @psalm-type TemporalConfig = array{
 *     defaultClient?: scalar|Param|null, // Default: "default"
 *     defaultScheduleClient?: scalar|Param|null, // Default: "default"
 *     workerFactory?: scalar|Param|null, // Default: "Temporal\\WorkerFactory"
 *     pool?: array{
 *         dataConverter?: scalar|Param|null,
 *         roadrunnerRPC?: scalar|Param|null,
 *     },
 *     workers?: array<string, array{ // Default: []
 *         maxConcurrentActivityExecutionSize?: scalar|Param|null, // To set the maximum concurrent activity executions this worker can have. // Default: 0
 *         workerActivitiesPerSecond?: float|Param, // Sets the rate limiting on number of activities that can be executed per second per worker. This can be used to limit resources used by the worker. Notice that the number is represented in float, so that you can set it to less than 1 if needed. For example, set the number to 0.1 means you want your activity to be executed once for every 10 seconds. This can be used to protect down stream services from flooding. // Default: 0
 *         taskQueue: scalar|Param|null,
 *         exceptionInterceptor?: scalar|Param|null, // Default: "temporal.exception_interceptor"
 *         finalizers?: list<scalar|Param|null>,
 *         interceptors?: list<scalar|Param|null>,
 *         maxConcurrentLocalActivityExecutionSize?: int|Param, // To set the maximum concurrent local activity executions this worker can have. // Default: 0
 *         workerLocalActivitiesPerSecond?: float|Param, // Sets the rate limiting on number of local activities that can be executed per second per worker. This can be used to limit resources used by the worker. Notice that the number is represented in float, so that you can set it to less than 1 if needed. For example, set the number to 0.1 means you want your local activity to be executed once for every 10 seconds. This can be used to protect down stream services from flooding. // Default: 0
 *         taskQueueActivitiesPerSecond?: int|Param, // Sets the rate limiting on number of activities that can be executed per second. This is managed by the server and controls activities per second for your entire task queue whereas WorkerActivityTasksPerSecond controls activities only per worker. Notice that the number is represented in float, so that you can set it to less than 1 if needed. For example, set the number to 0.1 means you want your activity to be executed once for every 10 seconds. This can be used to protect down stream services from flooding. // Default: 0
 *         maxConcurrentActivityTaskPollers?: int|Param, // Sets the maximum number of goroutines that will concurrently poll the temporal-server to retrieve activity tasks. Changing this value will affect the rate at which the worker is able to consume tasks from a task queue. // Default: 0
 *         maxConcurrentWorkflowTaskExecutionSize?: int|Param, // To set the maximum concurrent workflow task executions this worker can have. // Default: 0
 *         maxConcurrentWorkflowTaskPollers?: int|Param, // Sets the maximum number of goroutines that will concurrently poll the temporal-server to retrieve workflow tasks. Changing this value will affect the rate at which the worker is able to consume tasks from a task queue. // Default: 0
 *         enableSessionWorker?: bool|Param, // Session workers is for activities within a session. Enable this option to allow worker to process sessions. // Default: false
 *         sessionResourceId?: scalar|Param|null, // The identifier of the resource consumed by sessions. It's the user's responsibility to ensure there's only one worker using this resourceID. For now, if user doesn't specify one, a new uuid will be used as the resourceID. // Default: null
 *         maxConcurrentSessionExecutionSize?: int|Param, // Sets the maximum number of concurrently running sessions the resource support. // Default: 1000
 *         stickyScheduleToStartTimeout?: scalar|Param|null, // Optional: Sticky schedule to start timeout. The resolution is seconds. Sticky Execution is to run the workflow tasks for one workflow execution on same worker host. This is a optimization for workflow execution. When sticky execution is enabled, worker keeps the workflow state in memory. New workflow task contains the new history events will be dispatched to the same worker. If this worker crashes, the sticky workflow task will timeout after StickyScheduleToStartTimeout, and temporal server will clear the stickiness for that workflow execution and automatically reschedule a new workflow task that is available for any worker to pick up and resume the progress. // Default: null
 *         workerStopTimeout?: scalar|Param|null, // Optional: worker graceful stop timeout. // Default: null
 *         deadlockDetectionTimeout?: scalar|Param|null, // Optional: If set defines maximum amount of time that workflow task will be allowed to run. // Default: null
 *         maxHeartbeatThrottleInterval?: scalar|Param|null, // Optional: The default amount of time between sending each pending heartbeat to the server. This is used if the ActivityOptions do not provide a HeartbeatTimeout. Otherwise, the interval becomes a value a bit smaller than the given HeartbeatTimeout. // Default: null
 *     }>,
 *     clients?: array<string, array{ // Default: {"default":{"namespace":"default","address":"%env(TEMPORAL_ADDRESS)%","dataConverter":"temporal.data_converter","grpcContext":{"timeout":{"value":5,"format":"seconds"}},"interceptors":[]}}
 *         namespace: scalar|Param|null,
 *         address?: scalar|Param|null, // Default: {}
 *         identity?: scalar|Param|null,
 *         dataConverter?: scalar|Param|null, // Default: "temporal.data_converter"
 *         clientKey?: scalar|Param|null,
 *         clientPem?: scalar|Param|null,
 *         queryRejectionCondition?: 0|1|2|3|Param,
 *         interceptors?: list<scalar|Param|null>,
 *         grpcContext?: array{
 *             timeout?: array{
 *                 value?: int|Param, // Value connection timeout // Default: 5
 *                 format?: "nanoseconds"|"microseconds"|"milliseconds"|"seconds"|"minutes"|"hours"|"days"|"weeks"|"months"|"years"|Param, // Interval unit // Default: "seconds"
 *             },
 *             options?: list<mixed>,
 *             metadata?: list<mixed>,
 *             retryOptions?: array{
 *                 initialInterval?: scalar|Param|null, // Backoff interval for the first retry. // Default: null
 *                 maximumInterval?: scalar|Param|null, // Maximum backoff interval between retries. Exponential backoff leads to interval increase. This value is the cap of the interval. Example: 30 seconds // Default: null
 *                 backoff_coefficient?: float|Param, // Coefficient used to calculate the next retry backoff interval. The next retry interval is previous interval multiplied by this coefficient. Note: Must be greater than 1.0
 *                 maximumAttempts?: int|Param, // Maximum number of attempts. When exceeded the retries stop even if not expired yet. If not set or set to 0, it means unlimited
 *                 nonRetryableExceptions?: list<scalar|Param|null>,
 *             },
 *         },
 *     }>,
 *     scheduleClients?: array<string, array{ // Default: {"default":{"namespace":"default","address":"%env(TEMPORAL_ADDRESS)%","dataConverter":"temporal.data_converter","grpcContext":{"timeout":{"value":5,"format":"seconds"}},"interceptors":[]}}
 *         namespace: scalar|Param|null,
 *         address?: scalar|Param|null, // Default: {}
 *         identity?: scalar|Param|null,
 *         dataConverter?: scalar|Param|null, // Default: "temporal.data_converter"
 ...
 *     }>,
 * }
 * @psalm-type FreshCentrifugoConfig = array{
 *     jwt?: array{
 *         ttl?: int|Param, // TTL for JWT tokens in seconds. // Default: null
 *     },
 *     channel_max_length?: int|Param, // Maximum length of channel name. // Default: 255
 *     fake_mode?: bool|Param, // Enables fake mode for Centrifugo client, no real request will be sent. // Default: false
 *     api_key?: scalar|Param|null, // Centrifugo API key // Default: "%env(CENTRIFUGO_API_KEY)%"
 *     api_endpoint?: scalar|Param|null, // Centrifugo API endpoint // Default: "%env(CENTRIFUGO_API_ENDPOINT)%"
 *     secret?: scalar|Param|null, // Centrifugo secret // Default: "%env(CENTRIFUGO_SECRET)%"
 * }
 ...
 *     fresh_centrifugo?: FreshCentrifugoConfig,
 *     "when@dev"?: array{
 *         imports?: ImportsConfig,
 *         parameters?: ParametersConfig,
 *         services?: ServicesConfig,
